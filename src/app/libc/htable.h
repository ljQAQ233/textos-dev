...
static inline htable_t *htable_init(htable_t *ht, size_t size)
{
    if (!ht) ht = malloc(sizeof(htable_t));
    ht->size = size;
...
static inline void htable_add(htable_t *ht, hlist_node_t *node, htkey_t key)
{
...
#define htable_define(name, tblsz)                       \
    ...
    static htable_t name = {.size = (tblsz), .tbl = __htable_tbl(name)};
