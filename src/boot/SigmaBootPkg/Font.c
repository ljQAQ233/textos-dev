  ...
  )
{
  ...
  Status = FileOpen (Path, EFI_FILE_MODE_READ, &FontFile);
  ERR_RETS (Status);

  Status = FileAutoRead (FontFile, (VOID **)&Font->Base, &Font->Size);
  ERR_RETS (Status);
  ...
  )
{
  ...
  /* Locate the address of start of bitmap,this pointer points the start of
   * bitmap data,the size of bitmap in betys is (Width * Height / 8) */
  UINT8                          *Base  = Font.Base + Code * PerBmpSiz;
  EFI_GRAPHICS_OUTPUT_BLT_PIXEL  *Pixel =
    AllocatePages (EFI_SIZE_TO_PAGES (PerBmpSiz));
