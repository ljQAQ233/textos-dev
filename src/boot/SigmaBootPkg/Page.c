...
#define SIZE_TO_PAGES(Siz, PageSiz)         \
    ((Siz % PageSiz == 0) ? (Siz / PageSiz) \
                          : ((Siz - Siz % PageSiz) / PageSiz + 1))
  ...
  )
{
  ...
  for (Page = Pages; Page->IsValid; Page++) {
    Map (
      (UINT64)PML4,
  ...
  )
{
  ...
  UINT64  PageSiz =
    (Mode == MAP_4K) ? SIZE_4KB : ((Mode == MAP_2M) ? SIZE_2MB : SIZE_1GB);
  ...
  DEBUG ((
    DEBUG_INFO,
    "[INFO] Try to map 0x%llx -> 0x%llx - Mode : %u,size : %llu\n",
    PhyAddr,
    VirtAddr,
    Mode,
    Siz
  ...
  for (UINTN Idx = 0; Idx < PageNum;
       Idx++, PhyAddr += PageSiz, VirtAddr += PageSiz)
  {
    /*
    ...
    {
        DEBUG ((DEBUG_INFO,"[WARN] Page was mapped or dir was taken up! -
    Mode : %u - 0x%llx\n",Mode,VirtAddr)); Break(); goto Continue;
    }
    */

    if (~PML4[VIRT_PML4E_IDX (VirtAddr)] & PE_P) {
      // PDPT not found
    ...
    /* Set a 1GiB page */
    if (Mode == MAP_1G) {
      PDPT[VIRT_PDPTE_IDX (VirtAddr)] =
        PE_S_ADDR (PhyAddr) | Flgs | PDPTE_1G;
      goto Continue;
    }

    if (~PDPT[VIRT_PDPTE_IDX (VirtAddr)] & PE_P) {
      // PD not found
      PD = AllocatePages (1);
      ZeroMem (PD, SIZE_4KB);
    ...
    if (~PD[VIRT_PDE_IDX (VirtAddr)] & PE_P) {
      // PT not found
      PT = AllocatePages (1);
      ZeroMem (PT, SIZE_4KB);
...
Continue:
    // DEBUG ((DEBUG_INFO,"[DONE] Mapped 0x%llx -> 0x%llx - size :
  ...
  )
{
  ...
  return AsmWriteCr3 (PML4Addr | (Flgs & ~FlagsMask));
}
  ...
  )
{
  ...
  if (~PDPT[VIRT_PDPTE_IDX (VirtAddr)] & PE_P) {
    ...
    if (PDPT[VIRT_PDPTE_IDX (VirtAddr)] & PDPTE_1G) {
      DEBUG ((
        DEBUG_INFO,
        "[INFO] Page was mapped - 0x%llx, Type : 1GiB\n",
        VirtAddr
        ));
      return TRUE;
    }
  ...
  if (~PD[VIRT_PDE_IDX (VirtAddr)] & PE_P) {
    ...
    if (PD[VIRT_PDPTE_IDX (VirtAddr)] & PDE_2M) {
      DEBUG ((
        DEBUG_INFO,
        "[INFO] Page was mapped - 0x%llx, Type : 2MiB\n",
        VirtAddr
        ));
      return TRUE;
    }
  ...
  if (~PT[VIRT_PTE_IDX (VirtAddr)] & PE_P) {
    // PG not found
    goto NotFound;
  }
  ...
  DEBUG ((
    DEBUG_INFO,
    "[INFO] Mapped page was mapped - 0x%llx, Type : 4KiB\n",
    VirtAddr
    ));
  return TRUE;
...
NotFound:
  DEBUG (
    (DEBUG_INFO, "[INFO] Mapped page was not mapped - 0x%llx\n", VirtAddr));
  return FALSE;
}
  ...
  )
{
  ...
  for (UINT16 iPML4E = 0; iPML4E < 512; iPML4E++, PML4++) {
    // PML4 Level
    if (~*PML4 & PE_P) {
      VirtAddr += PDPT_SIZE;
      continue;
    ...
    for (UINT16 iPDPTE = 0; iPDPTE < 512; iPDPTE++, PDPT++) {
      // PDPT Level
      if (~*PDPT & PE_P) {
        VirtAddr += PD_SIZE;
        continue;
      ...
      if (*PDPT & PDPTE_1G) {
        DEBUG ((
          DEBUG_INFO,
          "[INFO] Mapped 0x%llx -> 0x%llx,Type : 1GiB\n",
          PE_V_ADDR (*PDPT),
          VirtAddr
          ));
        VirtAddr += PD_SIZE;
      ...
      for (UINT16 iPDE = 0; iPDE < 512; iPDE++, PD++) {
        // PD Level
        if (~*PD & PE_P) {
          VirtAddr += PT_SIZE;
          continue;
        ...
        if (*PD & PDE_2M) {
          DEBUG ((
            DEBUG_INFO,
            "[INFO] Mapped 0x%llx -> 0x%llx,Type : 2MiB\n",
            PE_V_ADDR (*PD),
            VirtAddr
            ));
          VirtAddr += PT_SIZE;
        ...
        for (UINT16 iPTE = 0; iPTE < 512; iPTE++, PT++) {
          // PT Level
          if (~*PT & PE_P) {
            VirtAddr += PAGE_SIZE;
            continue;
          ...
          DEBUG ((
            DEBUG_INFO,
            "[INFO] Mapped 0x%llx -> 0x%llx,Type : 4KiB\n",
            PE_V_ADDR (*PT),
            VirtAddr
            ));
          VirtAddr += PAGE_SIZE;
